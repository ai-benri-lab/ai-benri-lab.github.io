...
def write_lab_note(manifest):
    ...
    out_schema = ('{"title": "記事タイトル(38字以内、週表記入り。数字か学びが一目で分かる具体的な見出し。煽らない)", '
                  '"lead": "導入(130字。『AIが人手ゼロで動画+ブログを運営し、数字を全部公開する』という他に無い切り口を明示し、今週の要点を1つ先出しする)", '
                  '"sections": ['
                  '{"h": "今週の数字", "p": "220字。与えられたデータの実数を具体的に引用(再生の伸び・公開本数・GSC表示回数・クリック等)。前週比があれば触れる。丸めすぎない"}, '
                  '{"h": "今週試したことと、その結果", "p": "210字。今週行った具体的な施策や変更と、それが数字にどう出た/出なかったか。読者が自分でも真似できる粒度で書く"}, '
                  '{"h": "うまくいったこと・うまくいかなかったこと", "p": "200字。成功と失敗を両方、正直に。失敗と反省こそ読者価値が高い"}, '
                  '{"h": "来週やること", "p": "160字。次の具体的なアクションを箇条書き的に"}], '
                  '"conclusion": "締め(120字。AI無人運営の実録として次回も追う価値を示す)"}')
    prompt = "\n".join([
        "AIが完全無人で運営する動画チャンネル+ブログ「AIべんりラボ」の週次運営レポート記事"
        "（build in public）の素材をJSONで書いてください。",
        "この記事の唯一の価値は『AIが人手ゼロでメディアを運営し、成功も失敗も数字ごと包み隠さず公開する』"
        "という他に無い切り口です。読者(AI活用・自動化・副業・個人メディア運営に関心のある人)が"
        "『具体的で参考になった』と感じる、実データに基づく示唆を必ず入れてください。",
        "文体: 一人称は「当ラボ」。実数ベース・誇張や精神論なし・です・ます調。抽象的な決意表明で埋めない"
        "（例『頑張ります』ではなく『○○を△△に変えたら再生が□□に変化した』のように具体で書く）。",
